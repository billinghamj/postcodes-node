...
function Postcodes (api_key) {
	...
	this.api_url = "api.openpostcodes.com"
	this.version = "1";
}
...
Postcodes.prototype.lookupPostcode = function (postcode, callback) {
	...
	if (!this.api_key) {
		callback(new Error("Postcodes: No API Key provided"), null);
	}
...
Postcodes.prototype.postcodeForLocation = function (latitude, longitude, callback) {
	var options, url_params, request;
	...
	request = https.request(options, function (response) {
		...
		response.on('end', function () {
			...
			return callback(null, output.result[0]);
		});
	});
